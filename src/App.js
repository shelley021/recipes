...
// --- 核心配置 ---
const MY_DOMAIN = 'www.pocketfoxden.shop';
const IMAGE_OPTIMIZER_URL = 'https://images.weserv.nl/';

// 优化缩略图的函数
const getOptimizedThumbnailUrl = (originalUrl) => {
  if (originalUrl && typeof originalUrl === 'string' && !originalUrl.startsWith('data:')) {
    const proxiedUrl = `https://${MY_DOMAIN}/image-proxy/${originalUrl}`;
    return `${IMAGE_OPTIMIZER_URL}?url=${encodeURIComponent(proxiedUrl)}&w=300&output=webp&q=80`;
  }
  return originalUrl;
};

// 优化详情图的函数
const getOptimizedDetailUrl = (originalUrl) => {
  if (originalUrl && typeof originalUrl === 'string' && !originalUrl.startsWith('data:')) {
    const proxiedUrl = `https://${MY_DOMAIN}/image-proxy/${originalUrl}`;
    return `${IMAGE_OPTIMIZER_URL}?url=${encodeURIComponent(proxiedUrl)}&w=800&output=webp&q=90`;
  }
  return originalUrl;
};
...
function Modal({ recipe, onClose }) {
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className={`modal-content ${isMaximized ? 'modal-content-maximized' : ''}`} onClick={(e) => e.stopPropagation()}>
...
function Pagination({ currentPage, totalPages, onPageChange }) {
    ...
    return (
        <div className="pagination">
            <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1}>上一页</button>
            <span>第 {currentPage} / {totalPages} 页</span>
            <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages}>下一页</button>
            <input type="number" value={jumpPage} onChange={(e) => setJumpPage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleJump()} />
...
function App() {
    ...
    const [status, setStatus] = useState('正在从网络加载菜谱数据...');
    ...
    useEffect(() => {
        const dataUrl = `https://${MY_DOMAIN}/final_recipes_with_directions.json`;
        fetch(dataUrl)
            .then(response => { if (!response.ok) throw new Error('在线数据文件加载失败'); return response.json(); })
            .then(data => {
                setAllRecipes(data);
                setIsLoading(false);
    ...
    const searchRecipes = () => {
        ...
        if (isLoading) return;
        if (!keyword) { setFilteredRecipes([]); setStatus('请输入至少一种食材进行搜索。'); return; }
        setStatus('正在搜索...');
        ...
        if (results.length === 0) { setFilteredRecipes([]); setStatus('未找到包含所有指定食材的菜谱。'); return; }
        results.sort((a, b) => {
    ...
    return (
        <div>
            ...
            <div className="search-container">
                <input type="text" id="search" placeholder="输入食材..." onKeyDown={(e) => e.key === 'Enter' && searchRecipes()} disabled={isLoading} />
                <button onClick={searchRecipes} disabled={isLoading}>{isLoading ? '加载中...' : '搜索'}</button>
            </div>
